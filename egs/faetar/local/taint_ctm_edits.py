...
parser.add_argument(
    ...
    type=float,
    default=0.0,
    help="Segment confidence threshold below which the segment might be tainted",
)
parser.add_argument(
    "ctm_edits_in",
...
def ProcessUtterance(split_lines_of_utt, remove_deletions=True, conf_thresh=0.0):
    global num_lines_of_type, num_tainted_lines, num_del_lines_giving_taint, num_sub_lines_giving_taint, num_ins_lines_giving_taint
    ...
    for i in range(len(split_lines_of_utt)):
        ...
        if edit_type == "sil" or edit_type == "fix":
            ...
            edit_type == "cor" and split_lines_of_utt[i][4] != split_lines_of_utt[i][6]
        ):
            ...
            taintable[i] = True
        elif float(split_lines_of_utt[i][5]) < conf_thresh:
            taintable[i] = True
    ...
    for i in range(len(split_lines_of_utt)):
        if not remove_deletions or split_lines_of_utt[i][7] != "remove-this-line":
...
def ProcessData():
    try:
        ...
        sys.exit(
            "taint_ctm_edits.py: error opening ctm-edits input "
    ...
    try:
        ...
        sys.exit(
            "taint_ctm_edits.py: error opening ctm-edits output "
    ...
    while True:
        if len(split_pending_line) == 0 or split_pending_line[0] != cur_utterance:
            split_lines_of_cur_utterance = ProcessUtterance(
                split_lines_of_cur_utterance, args.remove_deletions, args.conf_thresh
            )
    ...
    try:
        ...
        sys.exit(
            "taint_ctm_edits.py: error closing ctm-edits output "
...
def PrintStats():
    ...
    if args.verbose < 1 or tot_lines == 0:
        return
    print(
        "taint_ctm_edits.py: processed {0} input lines, whose edit-types were: ".format(
            tot_lines
        )
        + ", ".join(
            [
    ...
    print(
        ...
        "lines were tainted."
        % (
            del_giving_taint_percent,
            sub_giving_taint_percent,
            ins_giving_taint_percent,
            tainted_lines_percent,
        ),
        file=sys.stderr,
    )
