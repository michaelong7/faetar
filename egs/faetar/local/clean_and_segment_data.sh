...
if [ $# = 5 ]; then
  ...
  echo "Usage: $0 [options] <data> <lang> <srcdir> [<calib-data>] <dir> <cleaned-data>"
  ...
  echo " By adjusting --conf-thresh to something > 0, one may make it more "
  echo " likely to remove words which decoding assigns low confidence. Confidence "
  echo " scores may be optionally calibrated to a partition with known "
  echo " segmentations by specifying the <calib-data> argument"
  echo ""
  echo "e.g. $0 --conf-thresh 0.5 data/train data/lang exp/tri3 data/dev exp/tri3_cleanup data/train_cleaned"
  echo "Options:"
  ...
  echo "  --conf-thresh <float>       # Confidence threshold below which to taint utterances. Default 0"
  exit 1
fi
...
if [ $stage -le 3 ]; then
  if [ ! -z "$calib_data" ]; then
    echo "$0: Calibrating confidence scores with biased LM..."
    if [ $data = "$calib_data" ]; then
      calib_graphs=$dir/graphs
      calib_latdir=$dir/lats
    else
      calib_graphs=$dir/calib_graphs
      calib_latdir=$dir/calib_lats
      steps/cleanup/make_biased_lm_graphs.sh $graph_opts \
        --nj $nj --cmd "$cmd" \
        $calib_data $lang $dir $calib_graphs
      steps/cleanup/decode_segmentation.sh \
        --beam 15.0 --nj $nj --cmd "$cmd --mem 4G" $transform_opt \
        --allow-partial false \
        $calib_graphs $calib_data $calib_latdir
    fi

    local/confidence_calibration.sh \
      --mdl "$dir" --data $calib_data --graph $calib_graphs \
      --latdir $calib_latdir --caldir "$dir/calibration"
  fi
fi
...
if [ $stage -le 4 ]; then
  ...
  if [ -d "$dir/calibration" ]; then
    local/lattice_oracle_align.sh \
      --cleanup $cleanup \
      --cmd "$cmd" $data $lang $dir/lats $dir/calibration $dir/lattice_oracle
  else
      local/lattice_oracle_align.sh \
        --cleanup $cleanup \
...
if [ $stage -le 7 ]; then
  ...
  $cmd $dir/log/taint_ctm_edits.log \
       local/taint_ctm_edits.py --conf-thresh $conf_thresh $dir/ctm_edits.modified $dir/ctm_edits.tainted
